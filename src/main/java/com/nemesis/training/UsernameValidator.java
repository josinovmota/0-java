...
public class UsernameValidator {
    ...
    public boolean validate () {
        ...
        if (!UNDERSIZED_USERNAMES.isEmpty()) {
            System.err.println("ERROR: These arguments must have a minimum length of 8 characters: " + UNDERSIZED_USERNAMES);
            hasErrors = true;
        }

        // Raises an error if there are oversized arguments being passed
        if (!OVERSIZED_USERNAMES.isEmpty()) {
            System.err.println("ERROR: These arguments must have a maximum length of 25 characters: " + OVERSIZED_USERNAMES);
            hasErrors = true;
        }
